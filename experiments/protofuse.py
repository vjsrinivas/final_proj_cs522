...
def run1(data_path):
    ...
    print("Fitting....")
    _model_1 = models.regressionTrees (_mini_train_pca, mini_y, _mini_test_pca, mini_test_y, max_depth=15)
    _model_2 = models.adaBoostRegression(_mini_train_pca, mini_y, _mini_test_pca, mini_test_y)
    del _mini_train_pca
    del mini_y
    del _mini_test_pca
    del mini_test_y
    ...
    test_result_1 = data.test(_model_1, test_x, is_scipy=True)
    test_result_2 = data.test(_model_2, test_x, is_scipy=True)
    np.save('test_1.npy', test_result_1)
    np.save('test_2.npy', test_result_2)
    exit()

    #np.save('test_out_example.npy', test_result)
    data.test_to_csv(test_result,'./submissions/test_proto9.csv')
